...
async function fetchSummaryData(ticker, crumb, headers) {
  const crumbParam = crumb ? `&crumb=${encodeURIComponent(crumb)}` : ''
  // UPDATED: Added incomeStatementHistory and cashflowStatementHistory
  // These are CRITICAL for calculating FCF manually when Yahoo doesn't provide it directly
  const modules = [
    'financialData',
...
export function transformYahooData(ticker, { quote, summary }) {
  ...
  const sd = summary?.summaryDetail || {}   // Added: summaryDetail module for avgVolume, dividends, etc.
  ...
  return {
    ...
    // avgVolume: summaryDetail is most reliable; quote meta and sd both carry this field
    ...
    marketCap: pickNumber(sd.marketCap?.raw, fd.marketCap?.raw, quote.marketCap),
      // FIXED: P/E with multiple fallbacks - trailingPE from financialData OR defaultKeyStatistics, then forwardPE
    ...
    // Yahoo returns ROE as decimal (0.15 for 15%), ensure consistency
    // Yahoo returns ROE as decimal (0.15 for 15%), ensure consistency
    // Yahoo returns ROE as decimal (0.15 for 15%), ensure consistency
    // FIXED: ROE with fallback paths - check both financialData and defaultKeyStatistics
    roe: pickNumber(fd.returnOnEquity?.raw, ks.returnOnEquity?.raw),
    roa: fd.returnOnAssets?.raw ?? null,
    // Net income needed for P/E calculation if trailingPE is null
    netIncome: fd.netIncome?.raw ?? null,
    ...
    ebitda: fd.ebitda?.raw ?? null,
      // FIXED: Calculate FCF manually if Yahoo doesn't provide it
    // FCF = Operating Cash Flow - Capital Expenditures
    freeCashFlow: fd.freeCashflow?.raw ?? calculateManualFCF(summary?.cashflowStatementHistory) ?? null,
    // Get OCF from financialData OR calculate from cashflow history
    ...
    // Aliases used by buildRiskPrompt in analyze/route.js
    ...
    _dataSources: {
      fcf: fd.freeCashflow?.raw ? 'yahoo_direct' : summary?.cashflowStatementHistory?.cashflowStatements?.[0]?.totalCashFromOperatingActivities?.raw ? 'calculated_from_cashflow' : 'missing',
      pe: fd.trailingPE?.raw ? 'financialData' : ks.trailingPE?.raw ? 'keyStatistics' : fd.forwardPE?.raw ? 'forwardPE_fallback' : 'missing',
      roe: fd.returnOnEquity?.raw ? 'financialData' : ks.returnOnEquity?.raw ? 'keyStatistics' : 'missing',
    }
  }
}
...
function calculateManualFCF(cashflowHistory) {
  ...
  // Get most recent statement (index 0)
