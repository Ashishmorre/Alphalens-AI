...
export function transformYahooData(ticker, { quote, summary }) {
	...
	const stockholderEquity = bs.stockholdersEquity?.raw
	...
	return {
		...
		tearningsGrowth: fd.earningsGrowth?.raw ?? null,
		...
		_dataSources: {
			fcf: fd.freeCashflow?.raw
				...
				: summary?.cashflowStatementHistory?.cashflowStatements?.[0]?.totalCashFromOperatingActivities?.raw
					...
					: fd.ebitda?.raw ? 'formula_ebitda_0.6' : 'missing',
			pe: fd.trailingPE?.raw ? 'financialData' : ks.trailingPE?.raw ? 'keyStatistics' : fd.forwardPE?.raw ? 'forwardPE_fallback' : 'missing',
			roe: fd.returnOnEquity?.raw ? 'financialData' : ks.returnOnEquity?.raw ? 'keyStatistics' : (netIncomeForROE && stockholderEquity) ? 'balance_sheet_formula' : 'missing',
		},
