...
export function validatePeers(ticker, peers) {
  ...
  if (!primaryInfo) {
    // Primary ticker not in our database — pass peers through without filtering
    // (sector-based rejection would incorrectly block all AI-returned peers)
    const validPeers = peers
      ...
      .map(p => {
        const t = typeof p === 'string' ? p : p?.ticker
        const info = getSectorInfo(t)
        return {
          ticker: t,
          original: p,
          sector: info?.sector || 'Unknown',
          name: info?.name || (typeof p === 'object' ? p?.name : null) || t,
        }
      })
    return {
      ...
      validPeers,
      rejectedPeers: [],
      errors: [`Primary ticker ${ticker} not in sector database — returning AI peers unfiltered`]
    }
  }
  ...
  for (const peer of peers) {
    ...
    if (!peerInfo) {
      // Peer not in our database — accept it (AI may know real peers we don't)
      validPeers.push({
        ticker: peerTicker,
        original: peer,
        sector: primarySector,
        name: typeof peer === 'object' ? peer?.name : peerTicker,
      })
      continue
    }
