...
import { getYahooFinanceData } from './yahoo-finance'
import { getNSEData } from './nse-xbrl-parser'
...
export async function runCompleteDCF(ticker, options = {}) {
  ...
  try {
    ...
    const terminalValue = calculateTerminalValue(finalFCF, wacc, terminalGrowth)
    ...
    const marketCap = cleanData.marketCap || cleanData.price * cleanData.sharesOutstanding
    const sector = (cleanData.sector || '').toLowerCase()
...
async function fetchWithFallback(ticker, signal) {
  ...
  let data = await getYahooFinanceData(ticker, signal)

  // Fallback to NSE if available
  if (!data || !data.revenue) {
    try {
      const nseData = await getNSEData(ticker)
      if (nseData) {
        data = { ...data, ...nseData }
      }
    } catch (e) {
