...
export function calculateEquityValue(enterpriseValue, cash, debt, marketCap, sector) {
  ...
  // Bull Market Floor: Align to 95% of current price for established companies
  if (equityValue < 0) {
    const floor = mCap ? mCap * 0.05 : Math.abs(ev) * 0.1
    return Math.max(floor, 0.01)
  }

  // Bull market floor: Don't value below 95% of market cap for quality companies
  return Math.max(equityValue, mCap * 0.95)
}
