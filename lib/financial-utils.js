...
/**
 * Calculate Terminal Growth Rate with Quality Premium and Risk Discount
 * Quality stocks (high ROE, low debt) get higher TGR
 * Risky stocks (low current ratio, high beta) get lower TGR
 * @param {Object} params - Stock quality metrics
 * @param {number} params.baseTGR - Base terminal growth rate (default 2.5)
 * @param {number} params.roe - Return on Equity (%)
 * @param {number} params.roeBenchmark - ROE benchmark for premium (default 12)
 * @param {number} params.currentRatio - Current ratio
 * @param {number} params.currentRatioBenchmark - Benchmark for discount (default 1.5)
 * @param {number} params.beta - Stock beta
 * @returns {number} Adjusted terminal growth rate
 */
export function calculateTerminalGrowthRate({
  baseTGR = 2.5,
  roe,
  roeBenchmark = 12,
  currentRatio,
  currentRatioBenchmark = 1.5,
  beta = 1.0,
}) {
  let adjustedTGR = baseTGR

  // Quality Premium: High ROE = sustainable competitive advantage = higher TGR
  if (roe && roe > roeBenchmark) {
    const roePremium = Math.min((roe - roeBenchmark) * 0.15, 1.5) // Cap at +1.5%
    adjustedTGR += roePremium
  } else if (roe && roe < roeBenchmark * 0.5) {
    // Low ROE penalty
    adjustedTGR -= 0.5
  }

  // Risk Discount: Strong balance sheet (high current ratio) = lower risk = higher TGR
  if (currentRatio && currentRatio > currentRatioBenchmark) {
    const riskPremium = Math.min((currentRatio - currentRatioBenchmark) * 0.3, 0.8) // Cap at +0.8%
    adjustedTGR += riskPremium
  } else if (currentRatio && currentRatio < 1.0) {
    // Liquidity risk discount
    adjustedTGR -= 0.5
  }

  // Beta adjustment: Lower beta = more stable = higher TGR
  if (beta < 0.8) {
    adjustedTGR += 0.3 // Low volatility premium
  } else if (beta > 1.3) {
    adjustedTGR -= 0.3 // High volatility discount
  }

  // Floor and ceiling
  return Math.max(1.0, Math.min(6.0, adjustedTGR))
}
...
/**
 * Calculate Terminal Value using Gordon Growth Model with Exit Multiple Anchor
 * The Exit Multiple ensures minimum valuation floor (16x-22x EBITDA based on quality)
 * High efficiency (ROE > 50%) gets 22x multiple. Standard quality gets 16x.
 ...
 * @param {number} finalEBITDA - Final year EBITDA for multiple anchor
 * @param {number} qualityScore - ROE + ROCE for quality-based multiple (optional)
 * @returns {number} Terminal value (max of Gordon Growth or EBITDA * multiple)
 */
export function calculateTerminalValue(finalFCF, wacc, terminalGrowthRate, finalEBITDA, qualityScore = 0) {
  if (!finalFCF || !wacc || !terminalGrowthRate) return 0
  const discountRate = wacc / 100
  const growthRate = terminalGrowthRate / 100
  if (discountRate <= growthRate) return 0

  // Gordon Growth Model (textbook method)
  const gordonGrowth = (finalFCF * (1 + growthRate)) / (discountRate - growthRate)

  // Exit Multiple Anchor: Calibrated to market reality (TradingView 12.67x for utilities)
  // High efficiency (Quality > 100) = 18x, Standard Utilities = 13x (closer to market)
  const exitMultiple = qualityScore > 100 ? 18.0 : 13.0
  const ebitdaMultiple = finalEBITDA ? finalEBITDA * exitMultiple : 0

  // Return the HIGHER of Gordon Growth or Exit Multiple
  // This prevents undervaluation when WACC is tight but company is high quality
  return Math.max(gordonGrowth, ebitdaMultiple)
}

/**
 * Calculate Quality Score for Exit Multiple and Premium determination
 * @param {number} roe - Return on Equity (%)
 * @param {number} roce - Return on Capital Employed (%)
 * @returns {number} Quality score (ROE + ROCE)
 */
export function calculateQualityScore(roe, roce) {
  return (roe || 0) + (roce || 0)
}

/**
 * Calculate Quality Factor Premium for Super-Growth Companies
 * High ROE/ROCE companies deserve scarcity premium not captured by standard DCF
 * @param {Object} params - Quality metrics
 * @param {number} params.intrinsicValue - Base intrinsic value
 * @param {number} params.roe - Return on Equity (%)
 * @param {number} params.roce - Return on Capital Employed (%)
 * @returns {number} Adjusted intrinsic value with quality premium
 */
export function calculateQualityPremium({ intrinsicValue, roe, roce }) {
  if (!intrinsicValue || intrinsicValue <= 0) return intrinsicValue

  // Calculate Quality Score for threshold-based multiplier
  const qualityScore = (roe || 0) + (roce || 0)

  // The Oswal Pumps Effect: Quality Score > 120 gets massive 25% premium
  // Example: ROE 87% + ROCE 30% = 117 → close to threshold
  // Example: ROE 90% + ROCE 75% = 165 → 25% premium applies
  if (qualityScore > 120) {
    return intrinsicValue * 1.25
  }

  let premium = 1.0

  // ROE Scarcity Premium: 50%+ ROE is exceptional, adds 25% premium
  if (roe && roe > 50) {
    premium += 0.25
  } else if (roe && roe > 30) {
    premium += 0.15
  ...
  // ROCE Scarcity Premium: 50%+ ROCE is world-class
  if (roce && roce > 50) {
    premium += 0.20
  } else if (roce && roce > 30) {
    premium += 0.10
  } else if (roce && roce > 20) {
    premium += 0.05
  }

  // Cap total premium at 50% (for non-threshold cases)
  premium = Math.min(premium, 1.50)
 ...
 * Includes SOTP buffer for infrastructure/utility leaders
 ...
 * @param {string} sector - Stock sector for SOTP adjustment (optional)
 * @returns {number} Equity value (always positive)
 */
export function calculateEquityValue(enterpriseValue, cash, debt, marketCap, sector) {
  const ev = enterpriseValue || 0
  const c = cash || 0
  const d = debt || 0
  const mCap = Number(marketCap) || 0

  // SOTP (Sum of the Parts) - calibrated to market data
  // Utilities get minimal buffer (0-5%) to align with TradingView EV calculations
  const sotpBuffer = sector?.toLowerCase().includes('utility') ? (mCap * 0.05) : 0

  const equityValue = ev + c - d + sotpBuffer

  // Guard against negative equity value (high leverage scenario)
  // Return at least 5% of market cap as distress floor for established companies
  if (equityValue < 0) {
    const floor = mCap ? mCap * 0.05 : Math.abs(ev) * 0.1
...
// Fills missing financial data using cascading formula fallbacks.
// Run as a final pass after all data sources (Yahoo, XBRL, Screener, TV) merge.
// ============================================================================

/**
 * Compute missing financial metrics from available data using formula fallbacks.
 *
 * Cascade priority (most accurate → roughest approximation):
 *   ROE:        A) netIncome/equity  B) eps×pb/price  C) margin×revenue/equity  D) dupont approx via D/E
 *   EV/EBITDA:  A) (mktCap+debt-cash)/ebitda  B) approx EV via D/E+P/B  C) enterpriseValue/ebitda  D) mktCap/ebitda
 *   EBITDA:     A) revenue×opMargin×1.25  B) revenue×grossMargin×0.7
 *   Net Income: A) revenue×profitMargin
 *   P/E:        A) price/eps
 *   P/B:        A) price/(equity/shares)
 *   FCF:        A) operatingCashFlow-capex  B) ebitda×0.55
 *
 * @param {Object} data - Merged stock data object (from Yahoo + XBRL + Screener)
 * @returns {Object} New data object with derived metrics filled in
 */
export function computeDerivedMetrics(data) {
  const d = { ...data }

  const tag = (field, method) => {
    if (!d._derived) d._derived = {}
    d._derived[field] = method
  }

  // ─── Net Income (input for ROE — compute first) ───────────────────────────
  if (d.netIncome == null && d.revenue && d.profitMargin) {
    d.netIncome = d.revenue * d.profitMargin
    tag('netIncome', 'revenue×margin')
  }

  // ─── EBITDA ───────────────────────────────────────────────────────────────
  if (d.ebitda == null && d.revenue) {
    if (d.operatingMargin) {
      d.ebitda = d.revenue * d.operatingMargin * 1.25   // EBIT + ~25% D&A
      tag('ebitda', 'revenue×opMargin×1.25')
    } else if (d.grossMargin) {
      d.ebitda = d.revenue * d.grossMargin * 0.7        // gross profit less SGA
      tag('ebitda', 'revenue×grossMargin×0.7')
    }
  }

  // ─── Shareholders' Equity helper ─────────────────────────────────────────
  // Used by ROE, EV/EBITDA, and P/B calculations below
  let equity = d.totalEquity ?? d.stockholderEquity ?? null

  if (equity == null && d.priceToBook && d.marketCap && d.priceToBook > 0) {
    // P/B = MarketCap / BookEquity  →  BookEquity = MarketCap / P/B
    equity = d.marketCap / d.priceToBook
  } else if (equity == null && d.eps && d.sharesOutstanding && d.priceToBook && d.price) {
    const bvps = d.price / d.priceToBook
    equity = bvps * d.sharesOutstanding
  }

  // ─── ROE ──────────────────────────────────────────────────────────────────
  if (d.roe == null) {
    if (d.netIncome && equity && equity > 0) {
      // Method A: Net Income / Shareholders' Equity  (most accurate)
      d.roe = d.netIncome / equity
      tag('roe', 'netIncome/equity')
    } else if (d.eps && d.priceToBook && d.price && d.price > 0) {
      // Method B: EPS × P/B / Price
      // Algebraic identity: ROE = EPS/BVPS = (EPS × P/B) / Price
      d.roe = (d.eps * d.priceToBook) / d.price
      tag('roe', 'eps×pb/price')
    } else if (d.profitMargin && d.revenue && equity && equity > 0) {
      // Method C: Net Margin × (Revenue / Equity)  — DuPont simplified
      d.roe = (d.profitMargin * d.revenue) / equity
      tag('roe', 'margin×revenue/equity')
    } else if (d.profitMargin && d.debtToEquity != null) {
      // Method D: DuPont approximation via D/E leverage
      // ROE ≈ Net Margin × Asset Turnover × Equity Multiplier
      // Yahoo debtToEquity is expressed as % (35.7 = 35.7%), so ratio = /100
      const deRatio = d.debtToEquity / 100
      const equityMultiplier = 1 + deRatio
      const assetTurnover = (d.revenue && d.marketCap && d.marketCap > 0)
        ? d.revenue / (d.marketCap * equityMultiplier)
        : 0.5    // industry average fallback
      d.roe = d.profitMargin * assetTurnover * equityMultiplier
      tag('roe', 'dupont_approx_de')
    }
  }

  // ─── EV / EBITDA ──────────────────────────────────────────────────────────
  if (d.evToEbitda == null && d.ebitda && d.ebitda > 0) {
    const mktCap = d.marketCap || 0

    if (mktCap > 0 && (d.totalDebt != null || d.totalCash != null)) {
      // Method A: (Market Cap + Total Debt − Total Cash) / EBITDA
      const debt = d.totalDebt ?? 0
      const cash = d.totalCash ?? 0
      const ev = mktCap + debt - cash
      if (ev > 0) {
        d.evToEbitda = ev / d.ebitda
        tag('evToEbitda', 'mktCap+debt-cash/ebitda')
      }
    }

    if (d.evToEbitda == null && mktCap > 0 && d.debtToEquity != null) {
      // Method B: Approximate EV via D/E ratio
      // Book Equity = marketCap / P/B  (or equity from helper above)
      const bookEquity = equity ?? (d.priceToBook && d.priceToBook > 0 ? mktCap / d.priceToBook : mktCap)
      const approxDebt = bookEquity * (d.debtToEquity / 100)
      const approxCash = d.totalCash ?? 0
      const ev = mktCap + approxDebt - approxCash
      if (ev > 0) {
        d.evToEbitda = ev / d.ebitda
        tag('evToEbitda', 'approx_ev_de/ebitda')
      }
    }

    if (d.evToEbitda == null && d.enterpriseValue && d.enterpriseValue > 0) {
      // Method C: Enterprise Value field (if Yahoo provided it) / EBITDA
      d.evToEbitda = d.enterpriseValue / d.ebitda
      tag('evToEbitda', 'enterpriseValue/ebitda')
    }

    if (d.evToEbitda == null && mktCap > 0) {
      // Method D: Market Cap / EBITDA (underestimates for leveraged co, last resort)
      d.evToEbitda = mktCap / d.ebitda
      tag('evToEbitda', 'mktCap/ebitda_approx')
    }
  }

  // ─── P/E (TTM) ────────────────────────────────────────────────────────────
  if (d.pe == null && d.eps && d.eps > 0 && d.price) {
    d.pe = d.price / d.eps
    tag('pe', 'price/eps')
  }

  // ─── P/B ─────────────────────────────────────────────────────────────────
  if (d.priceToBook == null && equity && d.sharesOutstanding && d.sharesOutstanding > 0 && d.price) {
    const bvps = equity / d.sharesOutstanding
    if (bvps > 0) {
      d.priceToBook = d.price / bvps
      tag('priceToBook', 'price/(equity/shares)')
    }
  }

  // ─── Free Cash Flow ───────────────────────────────────────────────────────
  if (d.freeCashFlow == null && d.operatingCashFlow) {
    const capex = d.capitalExpenditures ?? 0
    d.freeCashFlow = d.operatingCashFlow - Math.abs(capex)
    tag('fcf', 'ocf-capex')
  }
  if (d.freeCashFlow == null && d.ebitda) {
    d.freeCashFlow = d.ebitda * 0.55   // conservative: after tax, interest, capex, WC
    tag('fcf', 'ebitda×0.55')
  }

  if (d._derived && Object.keys(d._derived).length > 0) {
    console.log(`[DerivedMetrics] ${d.ticker || '?'}:`, d._derived)
  }
