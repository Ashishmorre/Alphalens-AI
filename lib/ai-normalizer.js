...
export function normalizeThesisData(raw) {
  ...
  // Validate key drivers - check if any have meaningful content
  const normalizedDrivers = normalizeArray(keyDrivers).map(normalizeDriver)
  const hasValidDrivers = normalizedDrivers.length > 0 &&
    normalizedDrivers.some(d => d.driver && d.driver !== 'Unknown' && d.driver !== '')
  ...
  if (!hasValidDrivers) {
    validationErrors.push('No valid investment drivers identified')
  }
  if (targetPrice === null) {
...
export function normalizeDCFData(raw) {
  ...
  const intrinsicValuePerShare = normalizePrice(data.intrinsicValuePerShare)
...
export function normalizeRiskData(raw) {
  ...
  return {
    ...
    peerBenchmarks: (data.peerBenchmarks && Array.isArray(data.peerBenchmarks))
      ? data.peerBenchmarks.map(normalizePeerBenchmark)
      : [],
  }
}
...
function normalizeRatio(val) {
  if (!val || typeof val !== 'object') return { name: null, value: '—', benchmark: null }
  return {
    name: normalizeString(val.name, null),
