...
function normalizeCase(val) {
  if (!val) return { ...DEFAULT_CASE }

  // Handle string responses (AI might return "Bullish scenario..." instead of object)
  ...
  const title = val.title || val.name || val.scenario || 'Scenario'
  const targetPrice = val.targetPrice || val.target || val.priceTarget || null
  const probability = val.probability || val.prob || val.chance || 33
  const points = val.points || val.keyPoints || val.bulletPoints || val.details || val.highlights || []
...
function normalizeDriver(val) {
  if (!val) return { driver: 'Unknown', detail: '', impact: 'NEUTRAL' }

  // Handle string responses (AI might return "Strong branding opportunities")
  if (typeof val === 'string') {
    return { driver: val, detail: '', impact: 'NEUTRAL' }
  }

  if (typeof val !== 'object') {
    return { driver: 'Unknown', detail: '', impact: 'NEUTRAL' }
  }

  // Flexible field extraction
  const driver = val.driver || val.name || val.factor || val.title || val.theme || 'Unknown'
  const detail = val.detail || val.description || val.reason || val.why || val.explanation || ''
  const impact = val.impact || val.effect || val.influence || 'NEUTRAL'

  return {
    driver: normalizeString(driver, 'Unknown'),
...
function normalizeStringArray(val) {
  ...
  if (typeof val === 'string' && val.trim()) {
    // Split comma-separated or newline-separated strings
