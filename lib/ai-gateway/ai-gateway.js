    ...
    for (let i = 0; i < tierModels.length; i++) {
      ...
      try {
        ...
        if (isPermissionError) {
          console.error(`[AIGateway] 🛑 PERMISSION DENIED for ${model}: Your API key does not have access to this model.`);
        } else {
        ...
        // If it was a permission error, don't wait - move to next immediately
        const waitTime = isPermissionError ? 10 : 100;
