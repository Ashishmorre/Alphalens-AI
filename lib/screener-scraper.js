...
export function mergeScreenerData(yahooData, screenerData) {
  ...
  if (screenerData.screenerRatios) {
    ...
    // Fill ROE: only if Yahoo is missing — never overwrite a valid Yahoo ROE
    if ((merged.roe === null || merged.roe === undefined) && sr.roe != null) {
      merged.roe = sr.roe / 100 // Screener gives %, convert to decimal for formatPct
    }
    ...
    // Prefer Screener EV/EBITDA for Indian stocks — more reliable than Yahoo formula
    if (sr.evToEbitda != null && sr.evToEbitda > 0) {
